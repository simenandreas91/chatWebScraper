...
def scrape() -> pd.DataFrame:
    ...
    for folder, files in sorted(grouped.items()):
        ...
        candidates = [files["README"], files["CODE"], repo_path]
...
def main():
    ...
    ap.add_argument("--out-xlsx", default="spreadsheets/inbound_actions.xlsx", help="Output .xlsx filename")
    ap.add_argument("--out-csv", default="spreadsheets/inbound_actions.csv", help="Output .csv filename")
    args = ap.parse_args()
    ...
    print(f"Saved {len(df)} inbound actions to {args.out_xlsx}")
    if args.out_csv:
