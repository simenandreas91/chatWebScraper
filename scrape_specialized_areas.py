...
# Repo folder that holds all specialized areas scripts
FOLDER = "Specialized Areas"
...
def group_specialized_areas_files(tree: List[Dict]) -> Dict[str, Dict[str, Dict[str, str]]]:
    """
    Group specialized areas script files by type_folder > snippet_folder.
    Each snippet_folder represents a specialized area snippet.
    """
    ...
    for node in tree:
        ...
        rel = path[len(prefix):]  # e.g., "CMDB/Example Name/README.md"
        parts = rel.split("/")
        if len(parts) < 3:
            continue
...
def scrape() -> pd.DataFrame:
    ...
    grouped = group_specialized_areas_files(tree)
...
def main():
    ap = argparse.ArgumentParser(description="Scrape Specialized Areas from code-snippets repo")
    ap.add_argument("--out-xlsx", default="spreadsheets/specialized_areas.xlsx", help="Output .xlsx filename")
    ap.add_argument("--out-csv", default="spreadsheets/specialized_areas.csv", help="Output .csv filename")
    args = ap.parse_args()
    ...
    print(f"Saved {len(df)} specialized areas to {args.out_xlsx}")
    if args.out_csv:
