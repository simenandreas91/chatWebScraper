...
def main():
    ...
    ap.add_argument("--out", default="mail_scripts.xlsx", help="Output .xlsx filename")
    args = ap.parse_args()

    df = scrape()
    df = df[[
        "name", "description", "code1", "code2", "repo_path"
    ]]

    df.to_excel(args.out, index=False)
    print(f"Saved {len(df)} mail scripts to {args.out}")
