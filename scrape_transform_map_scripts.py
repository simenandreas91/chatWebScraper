...
# Repo folder that holds all transform map scripts
FOLDER = "Server-Side Components/Transform Map Scripts"
...
def group_transform_map_scripts_files(tree: List[Dict]) -> Dict[str, Dict[str, str]]:
    """
    Group transform map script files by subfolder.
    Each subfolder represents a transform map script snippet.
    """
...
def scrape() -> pd.DataFrame:
    ...
    grouped = group_transform_map_scripts_files(tree)
...
def main():
    ap = argparse.ArgumentParser(description="Scrape Transform Map Scripts from code-snippets repo")
    ap.add_argument("--out-xlsx", default="spreadsheets/transform_map_scripts.xlsx", help="Output .xlsx filename")
    ap.add_argument("--out-csv", default="spreadsheets/transform_map_scripts.csv", help="Output .csv filename")
    args = ap.parse_args()
    ...
    print(f"Saved {len(df)} transform map scripts to {args.out_xlsx}")
    if args.out_csv:
