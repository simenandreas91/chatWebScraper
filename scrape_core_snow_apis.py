...
# Repo folder that holds all core ServiceNow APIs scripts
FOLDER = "Core ServiceNow APIs"
...
def group_core_apis_files(tree: List[Dict]) -> Dict[str, Dict[str, Dict[str, str]]]:
    """
    Group core APIs script files by api_folder > snippet_folder.
    Each snippet_folder represents a core API snippet.
    """
    ...
    for node in tree:
        ...
        rel = path[len(prefix):]  # e.g., "GlideRecord/Example Name/README.md"
        parts = rel.split("/")
        if len(parts) < 3:
            continue
        ...
        low = filename.lower()
        if low == "readme.md":
            grouped[api_folder][snippet_folder]["README"] = path
        elif low.endswith(".js"):
            files = grouped[api_folder][snippet_folder]
            if not files["CODE1"]:
                files["CODE1"] = path
...
def build_row(api_folder: str, name: str, files: Dict[str, str]) -> Dict[str, str]:
    ...
    repo_path = f"{FOLDER}/{api_folder}/{name}"
...
def scrape() -> pd.DataFrame:
    ...
    grouped = group_core_apis_files(tree)

    rows = []
    for api_folder in sorted(grouped.keys()):
        for snippet_folder, files in sorted(grouped[api_folder].items()):
            if not files["README"]:
                continue
            repo_path = f"{FOLDER}/{api_folder}/{snippet_folder}"
            candidates = [
                files["README"],
                files["CODE1"],
                files["CODE2"],
            ...
            if not is_recent(candidates):
                continue
            rows.append(build_row(api_folder, snippet_folder, files))

    return pd.DataFrame(rows)


def main():
    ap = argparse.ArgumentParser(description="Scrape Core ServiceNow APIs from code-snippets repo")
    ap.add_argument("--out-xlsx", default="spreadsheets/core_snow_apis.xlsx", help="Output .xlsx filename")
    ap.add_argument("--out-csv", default="spreadsheets/core_snow_apis.csv", help="Output .csv filename")
    args = ap.parse_args()
    ...
    print(f"Saved {len(df)} core ServiceNow APIs to {args.out_xlsx}")
    if args.out_csv:
